"""P2 pyramid + conventional C-style detection towers. Default: scratch training."""
import argparse
from pathlib import Path
import ultralytics
from p2_plain_structure import P2PlainYOLO
from train_yolov8n_s0 import TRAIN_ARGS, DATA
from train_yolov8n_mobilenetv2_w0625_s0 import VAL_ARGS

MODEL = Path(__file__).with_name('yolov8n_mobilenetv2_w0625_p2_plain.yaml')
NAME = 'yolov8n_mobilenetv2_w0625_p2_plain_640_s0_scratch'
...
def main():
    ...
    print(f'Model: {MODEL}\nRun: {run}',flush=True)
    if args.check:
        from check_p2_plain import check
    ...
    if args.resume or args.val or args.test:
        ...
        model=P2PlainYOLO(str(checkpoint),task='detect')
        if model.model.yaml.get('p2_plain_design')!='c_style_head_v1':
        ...
        P2PlainYOLO(str(MODEL),task='detect').train(**train_args)
